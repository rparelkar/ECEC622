...
int gaussianCruncherByOpenMp(const Matrix A, Matrix U);
int thread_count;
int main(int argc, char** argv) {
	...
	printf("CPU run time for serial code = %0.2f s. \n",
	...
	if (status == 0) {
		printf(
				"Failed to convert given matrix to upper triangular. Try again. Exiting. \n");
		exit(0);
	}
	...
	gettimeofday(&stop, NULL);
	printf("CPU run time for parallel code = %0.2f s. \n",
			(float) (stop.tv_sec - start.tv_sec
					+ (stop.tv_usec - start.tv_usec) / (float) 1000000));
	...
	//newValue
 	...
 	float speedup = (oldValue)/(newValue);
...
void gauss_eliminate_using_openmp(const Matrix AR, Matrix UR) /* Write code to perform gaussian elimination using OpenMP. */
{
	int i, j;
	float *U = UR.elements;
	float *A = AR.elements;
	int num_elements = AR.num_rows;
...
#pragma omp parallel for num_threads(thread_count) private(i) schedule(static)
	...
	print_matrix(UR);
#pragma omp parallel for num_threads(thread_count) private(i) schedule(static)
	for (i = 0; i < AR.num_rows; i++) {
		gaussianCruncherByOpenMp(AR, UR); // Each thread performs an unpredictable amount of work per iteration
	}
#pragma omp parallel for num_threads(thread_count) private(i) schedule(static)
	for (i = 0; i < 8; i++) {
...
int gaussianCruncherByOpenMp(const Matrix AR, Matrix UR) {
	printf("Thread cruncher %d executes loop iteration  \n",
			omp_get_thread_num());
...
float get_random_number(int min, int max) { /* Returns a random FP number between min and max values. */
	return (float) floor(
			(double) (min
					+ (max - min + 1) * ((float) rand() / (float) RAND_MAX)));
}

int perform_simple_check(const Matrix M) { /* Check for upper triangular matrix, that is, the principal diagonal elements are 1. */
...
void print_matrix(const Matrix A) {
	...
	for (i = 0; i < A.num_columns; i++) {
		for (j = 0; j < A.num_rows; j++) {
			printf(" %f ", A.elements[(i * A.num_columns) + j]);
		}
		printf(" \n");
	}
